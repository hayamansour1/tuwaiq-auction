...
function JoinQr({ centered = false }: { centered?: boolean }) {
  return (
    <div className={centered ? "mx-auto mt-6 w-fit" : "mt-4 flex justify-start"}>
    <div
      className={
        centered
          ? "mx-auto mt-6 w-fit"
    ...
    >
        {centered && (
          <p className="mb-3 text-xs font-bold text-[#4F29B7]">
            امسح للمشاركة
          </p>
        )}

        <QRCodeSVG value={AUCTION_URL} size={centered ? 110 : 48} />
      </div>
    </div>
  );
}
